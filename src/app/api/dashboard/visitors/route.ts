...
import { fetchGA4Visitors } from '@/lib/ga4'
...
export async function GET(request: Request) {
  ...
  const cacheKey = makeCacheKey(requestedClientId, 'visitors', `${days}d`)
  ...
  try {
    const data = await fetchGA4Visitors(config.ga4PropertyId, config.ga4ServiceAccountJson, days)
