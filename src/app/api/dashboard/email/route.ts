...
export async function GET(request: Request) {
  ...
  if (!config?.emailOctopusApiKey) {
    return NextResponse.json({ data: null, lastUpdated: null, error: 'Email Octopus not configured' }, { status: 503 })
  }

  try {
    const data = await fetchEmailOctopusMetrics(config.emailOctopusApiKey, days)
    ...
    return NextResponse.json({ data: null, lastUpdated: null, error: 'Failed to fetch Email Octopus data' }, { status: 503 })
  }
}
